        ...
        <Paper elevation={8} sx={{ p: 5, bgcolor: '#fff', borderRadius: 4 }}>
          ...
          >
            ...
            <Typography variant="body1" sx={{ color: '#1976d2', fontWeight: 'bold' }}>
              {isDragActive ? '여기에 파일을 놓으세요!' : '이미지를 드래그하거나 아래 버튼으로 선택하세요.'}
            </Typography>
            <Typography variant="body2" sx={{ color: '#555', mt: 1 }}>
              {file ? `선택된 파일: ${file.name}` : '이미지 파일만 업로드 가능합니다.'}
            </Typography>
            <Button variant="contained" onClick={open} sx={{ mt: 2, bgcolor: '#1976d2', '&:hover': { bgcolor: '#1565c0' } }}>
              파일 선택
            </Button>
          </Box>
          ...
          <Button
            ...
            sx={{ fontWeight: 'bold', fontSize: 18, py: 1.5, mb: 2, bgcolor: '#43a047', '&:hover': { bgcolor: '#388e3c' } }}
          >
          ...
          {qrUrl && (
            <Box sx={{ textAlign: 'center' }}>
              ...
              <img
                src={qrUrl}
                alt="QR 코드"
                style={{ width: 256, height: 256, padding: 10, borderRadius: 8, border: '1px solid #eee', background: '#fff' }}
              />
              <Button
                href={qrUrl}
                download="qr.png"
                variant="outlined"
                sx={{ mt: 2, color: '#1976d2', borderColor: '#1976d2', fontWeight: 'bold', '&:hover': { bgcolor: '#e3f2fd' } }}
              >
                다운로드
              </Button>
            </Box>
